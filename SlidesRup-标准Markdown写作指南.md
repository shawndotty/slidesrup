...
## 13. Grid（drag / drop）宽度/高度写法（可选）

如果你在模板或页面里直接使用 Advanced Slides 的 `<grid>` 元素（带 `drag` / `drop` 属性），宽度/高度（也就是 `drag` 的前两个值）支持两种写法：

- 纯数字：表示“比例模式”（整数），最终按百分比计算
- 带单位：以 `px` 结尾的像素值（正整数），最终会按像素参与计算（在非 absolute 模式下会换算为百分比）

示例（比例模式）：

```html
<grid drag="5 12" drop="0 0">
</grid>
```
...
```html
<grid drag="200px 300px" drop="0 0">
</grid>
```

校验规则：

- 像素模式：必须是正整数（例如 `200px`，不允许 `0px`、`-10px`、`12.5px`）
- 比例模式：必须是整数（例如 `5`，不允许 `5.5`）
