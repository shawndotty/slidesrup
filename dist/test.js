...
// src/__tests__/mock-obsidian.ts
var moment = () => ({ format: () => "" });
...
function _get_current_lang() {
  ...
  try {
    ...
    return slidesRupRunningLanguage === "ob" ? moment.locale() : slidesRupRunningLanguage;
  } catch (e) {
    console.error(
...
function generateGridBlock(block) {
  const childrenStr = (block.children || []).map(
    (child) => child.type === "grid" ? generateGridBlock(child) : child.raw.trim()
  ).filter(Boolean).join("\n\n");
...
import_module.default.prototype.require = function(...args) {
  if (args[0] === "obsidian")
    return {};
