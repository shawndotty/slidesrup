...
exports.renderDesignCanvas =
	exports.renderDesignToolbar =
	exports.applyGridFlexStyles =
	exports.applyBlockRectStyles =
	exports.computePanForZoom =
	exports.computeCanvasTransform =
	exports.computeBaselineScale =
	exports.clampCanvasZoomPercent =
		void 0;
...
function renderDesignToolbar(options) {
	var container = options.container,
		...
		hasFootnotesBlock = options.hasFootnotesBlock,
		hasSideBarBlock = options.hasSideBarBlock,
		_a = options.rectUnit,
		rectUnit = _a === void 0 ? "percent" : _a,
		_b = options.slideBaseWidth,
		slideBaseWidth = _b === void 0 ? DESIGN_MAKER_SLIDE_WIDTH : _b,
		_c = options.slideBaseHeight,
		slideBaseHeight = _c === void 0 ? DESIGN_MAKER_SLIDE_HEIGHT : _c,
		onAddBlock = options.onAddBlock,
		onAddFootnotes = options.onAddFootnotes,
		onAddSideBar = options.onAddSideBar,
	...
	[
		...
		var kind = _a[0],
			label = _a[1];
		var button = toolbar.createEl("button", {
			text: (0, helpers_1.t)(label),
		});
		...
		button.addEventListener("dragstart", function (e) {
			if (e.dataTransfer) {
				e.dataTransfer.setData(
					"application/json",
					JSON.stringify({ action: "add", kind: kind }),
				);
			}
		});
	...
	footnotesButton.addEventListener("click", function () {
		return onAddFootnotes();
	});
...
function createTemplateBlock(kind, rectUnit, slideBaseWidth, slideBaseHeight) {
	...
	var percentRect = { x: 10, y: 10, width: 40, height: 20 };
	var rect =
		rectUnit === "px"
			? {
	...
	return {
		id: "grid-"
			.concat(Date.now(), "-")
			.concat(Math.random().toString(16).slice(2, 8)),
		type: "grid",
		role: kind,
		rect: rect,
...
function renderDesignCanvas(options) {
	...
	var app = options.app,
		...
		page = options.page,
		_b = options.themeRawCss,
		themeRawCss = _b === void 0 ? "" : _b,
		_c = options.presentationCss,
		presentationCss = _c === void 0 ? "" : _c,
		_d = options.slideBaseWidth,
		slideBaseWidth = _d === void 0 ? DESIGN_MAKER_SLIDE_WIDTH : _d,
		_e = options.slideBaseHeight,
		slideBaseHeight = _e === void 0 ? DESIGN_MAKER_SLIDE_HEIGHT : _e,
		_f = options.canvasZoomPercent,
		canvasZoomPercent = _f === void 0 ? 100 : _f,
		_g = options.canvasPanX,
		canvasPanX = _g === void 0 ? 0 : _g,
		_h = options.canvasPanY,
		canvasPanY = _h === void 0 ? 0 : _h,
		_j = options.isPanKeyDown,
		isPanKeyDown =
			_j === void 0
				? function () {
						return false;
		...
		onPatchBlock = options.onPatchBlock,
		onAddBlock = options.onAddBlock,
		onDeleteBlock = options.onDeleteBlock,
		onDuplicateBlock = options.onDuplicateBlock;
	var rectUnit =
		(_a = page.rectUnit) !== null && _a !== void 0 ? _a : "percent";
	...
	frame.style.aspectRatio = ""
		.concat(slideBaseWidth, " / ")
		.concat(slideBaseHeight);
	...
	var renderBlock = function (parentEl, block) {
		...
		if (block.type === "raw") {
			...
			var result_1 = (0, design_block_renderer_1.renderBlockContent)(
				raw,
				block.raw,
				{
					app: app,
		...
		var result = (0, design_block_renderer_1.renderBlockContent)(
			el,
			block.content || "",
			{
				app: app,
		...
		if (
			!result.rendered &&
			!((_a = block.content) === null || _a === void 0
				? void 0
				: _a.trim())
		) {
	...
	page.blocks.forEach(function (block) {
		return renderBlock(canvas, block);
	});
	applyCanvasTransform();
}
