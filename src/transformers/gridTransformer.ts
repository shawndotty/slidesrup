...
export class GridTransformer implements AttributeTransformer {
	...
	readValues(
		...
		try {
			...
			const width = dragMatch?.[1] as string | undefined;
			const height = dragMatch?.[2] as string | undefined;
			const x = dropMatch?.[1] as string | undefined;
	...
	leftOf(grid: Map<string, number>): number {
		if ((grid.get("x") as number) < 0) {
			return (
				(grid.get("maxWidth") as number) +
				(grid.get("x") as number) -
				(grid.get("width") as number)
			);
		} else {
			return grid.get("x") as number;
		}
	}

	topOf(grid: Map<string, number>): number {
		if ((grid.get("y") as number) < 0) {
			return (
				(grid.get("maxHeight") as number) +
				(grid.get("y") as number) -
				(grid.get("height") as number)
			);
		} else {
			return grid.get("y") as number;
		}
	}

	heightOf(grid: Map<string, number>): number {
		return grid.get("height") as number;
	}

	widthOf(grid: Map<string, number>): number {
		return grid.get("width") as number;
	}
}
