...
exports.syncDraftRawMarkdown =
	exports.generateThemeCss =
	exports.generatePageMarkdown =
		void 0;
...
function serializeAttributes(block, defaultRectUnit) {
	var rectUnit =
		block.extraAttributes && block.extraAttributes.rectUnit === "px"
			? "px"
			: defaultRectUnit;
	var attrs = {
		drag:
			rectUnit === "px"
				? ""
						.concat(formatRectValue(block.rect.width), "px ")
						.concat(formatRectValue(block.rect.height), "px")
				: ""
						.concat(formatRectValue(block.rect.width), " ")
						.concat(formatRectValue(block.rect.height)),
		drop:
			rectUnit === "px"
				? ""
						.concat(formatRectValue(block.rect.x), "px ")
						.concat(formatRectValue(block.rect.y), "px")
				: ""
						.concat(formatRectValue(block.rect.x), " ")
						.concat(formatRectValue(block.rect.y)),
	};
...
function generateGridBlock(block, defaultRectUnit) {
	...
	var innerContent = [block.content, childrenStr]
		...
		.join("\n\n");
	return "<grid "
		.concat(serializeAttributes(block, rectUnit), ">\n")
		.concat(innerContent, "\n</grid>");
}
function generatePageMarkdown(page) {
	var rectUnit =
		(page === null || page === void 0 ? void 0 : page.rectUnit) ||
		"percent";
	return page.blocks
		.map(function (block) {
...
function generateThemeCss(theme, designName) {
	...
	var generatedCss = "\n:root {\n\t--sr-dm-primary: "
		.concat(theme.primaryColor, ";\n\t--sr-dm-secondary: ")
		.concat(theme.secondaryColor, ";\n\t--sr-dm-background: ")
		.concat(theme.backgroundColor, ";\n\t--sr-dm-text: ")
		.concat(theme.textColor, ";\n\t--sr-dm-heading-font: ")
		.concat(theme.headingFont, ";\n\t--sr-dm-body-font: ")
		.concat(theme.bodyFont, ";\n\t--sr-dm-heading-scale: ")
		.concat(theme.headingScale, ";\n\t--sr-dm-body-scale: ")
		.concat(theme.bodyScale, ";\n\t--sr-dm-radius: ")
		.concat(theme.borderRadius, "px;\n\t--sr-dm-shadow-opacity: ")
		.concat(theme.shadowOpacity, ";\n\t--sr-dm-mode: ")
		.concat(
			theme.mode,
